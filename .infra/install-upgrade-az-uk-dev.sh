...
helm upgrade --install --namespace ${K8S_DHOS_URL_API_NAMESPACES} gdm-articles-api $(dirname ${BASH_SOURCE})/helm-chart/gdm-articles-api \
     ...
     -f $(dirname ${BASH_SOURCE})/helm-chart/gdm-articles-api/values-az-uk-dev.yaml \
     -f <(sops --decrypt $(dirname ${BASH_SOURCE})/helm-chart/gdm-articles-api/values-az-uk-dev-secrets-sops.yaml)
